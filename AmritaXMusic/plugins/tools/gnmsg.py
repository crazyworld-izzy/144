...
async def goodnight_command_handler(_, message):
    ...
    if send_sticker:
        ...
        await message.reply_text(f"Goodnight, {sender}! Sleep tight. 🌙")  # Await the async call
    else:
        emoji = get_random_emoji()
        await app.send_message(message.chat.id, emoji)  # Await the async call
        await message.reply_text(f"Goodnight, {sender}! Sleep tight. {emoji}")  # Await the async call
